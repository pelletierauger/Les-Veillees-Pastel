...
//     c = c - smoothstep(r * 0.5, r * 0.5 - 0.1, d);
    ...
    float siz2 = sin(t * 50.);
    p2 = vec2(0.0, 0.0);
    vec3 col = CircleRGB(uv, p2, 0.8, 0.2, vec3(0.7, 0.5, 1.0));
    col *= sin(length(uv - p) * (30. + sin(t * 20.) * 100.));
    vec2 newV = uv - p;
//     col *= sin(atan(newV.x, newV.y) * (14. + floor(sin(t * 10.) * 10.)));
    col *= sin(sin(atan(newV.x, newV.y) * 3.) + sin(t * 200.));
    col += CircleRGB(uv, p2 - vec2(0.0, 0.4), 0.8, 0.2, vec3(0.0, 0.5, 1.0));

...
//     c = c - smoothstep(r * 0.5, r * 0.5 - 0.1, d);
    ...
    float siz2 = sin(t * 50.);
    p2 = vec2(0.0, 0.0);
    vec3 col = CircleRGB(uv, p2, 0.8, 0.5, vec3(0.7, 0.5, 1.0));
    float ond = length(uv - p) * 0.5;
//     col *= sin(ond * (0.1 + sin(t * 2.) * 300.));
      col -= abs(sin(pow(ond, 0.1) * 800. * sin(t * 100.)) * sin(t * 10.)) * (cos(ond * 16.) + sin(ond));
     col -= abs(sin(pow(ond, 0.14) * 8.) * 2.5) * (cos(ond) + sin(ond));
    vec2 newV = uv - p;
    float at = atan(newV.x, newV.y);
    col += sin(sin(at * 5.) * cos(at * 5.) * 5.);
     col.x -= abs(cos(1. + sin(2. + at * 4.) * cos(at * 3.) * 1.));
//      col *= sin(atan(newV.x, newV.y) * (2. + floor(sin(t * 10.) * 10.)));
//     col *= sin(sin(atan(newV.x, newV.y) * 4.) + sin(t * 100.));
        col = normalize(col);
         col += CircleRGB(uv, p2, 0.8, 0.2, vec3(0.0, 1.0, 0.1) * 0.5);
